...
| Bottom gallery of `ImageView` thumbnails → `addObject()` at screen centre | "Add marker" → tap a surface → name it and bind a device | Placement moved from the crosshair to the tap; identity is chosen after placement instead of before |
...
| `ThreeAxisController` d-pad nudging `localPosition` by 0.1 m | SceneView node gestures: drag re-hit-tests planes, twist rotates, pinch scales | The d-pad was a stand-in for gestures that Sceneform made awkward |
